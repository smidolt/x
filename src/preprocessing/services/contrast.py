"""Standalone contrast enhancement step with direct implementation."""
...
def run(payload: Dict[str, object]) -> Dict[str, object]:
    ...
    result = steps.enhance_contrast(image)
    elapsed = time.perf_counter() - start

    output_path = output_dir / f"{image_path.stem}__contrast.png"
    save_kwargs = {}
    ...
    LOGGER.info(
        "contrast applied=%s elapsed=%.2fs output=%s warning=%s",
        result.applied,
        elapsed,
        output_path,
        result.warning,
