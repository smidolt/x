"""Standalone grayscale step with direct implementation."""
...
def run(payload: Dict[str, object]) -> Dict[str, object]:
    ...
    result = steps.to_grayscale(image)
    elapsed = time.perf_counter() - start

    output_path = output_dir / f"{image_path.stem}__grayscale.png"
    save_kwargs = {}
    ...
    LOGGER.info(
        "grayscale applied=%s elapsed=%.2fs output=%s warning=%s",
        result.applied,
        elapsed,
        output_path,
        result.warning,
