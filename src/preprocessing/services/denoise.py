"""Standalone denoise step with direct implementation."""
...
def run(payload: Dict[str, object]) -> Dict[str, object]:
    ...
    strength = str(params.get("strength", "medium"))
    ...
    result = steps.denoise_image(image, strength)
    elapsed = time.perf_counter() - start

    output_path = output_dir / f"{image_path.stem}__denoise.png"
    save_kwargs = {}
    ...
    LOGGER.info(
        "denoise applied=%s elapsed=%.2fs output=%s warning=%s",
        result.applied,
        elapsed,
        output_path,
        result.warning,
